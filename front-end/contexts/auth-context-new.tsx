    ...
    const initAsgardeo = async () => {
      try {
        if (typeof window !== 'undefined') {
          ...
          const { asgardeoConfig } = await import("@/config/asgardeo.config")
  ...
  const login = async (): Promise<void> => {
    try {
      ...
      if (err instanceof Error) {
        ...
        } else if (err.message.includes("Authentication not initialized")) {
          errorMessage = "Authentication service is still loading. Please try again in a moment."
        }
      }
